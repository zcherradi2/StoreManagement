...
export function Modal<T extends Model>(props: ModalProps<T>) {
    ...
    const closeModal = () => {
        ...
        root.setIsModalOpen(false);
        root.history.current = [];
    };
    const modalGoBack = () => {
        console.log('History before pop:', root.history.current);

        const previousCard:Card|undefined = root.history.current.pop()
        root.setSelectedItem(null)
        ...
        if (previousCard == null || previousCard == undefined) {
            ...
            root.setSelectedCard(previousCard);
    ...
    const getCardsAndTitle = ():[string,{[key: string]: Card}|null]=>{
        const selectedModal = root.selectedCard?.route
                ...
                <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center">
                    <div className={`bg-white rounded-lg shadow-lg w-7/12 h-150 border-2 border-gray-200`}>
                        ...
                        <div className="flex justify-between items-center bg-gray-100 px-4 py-2 rounded-t-lg border-b border-gray-200 w-full">
                            <h2 className="font-semibold text-lg">{root.selectedCard?.title}</h2>
                        ...
                        <div className='w-full min-h-150 p-6 bg-white rounded-lg'>
                            {(() => {
                                const modalType = root.selectedCard?.modalType ? root.selectedCard.modalType : 1
                                switch (modalType) {
                                    case 1:
                                    ...
                                    case 2:
                                        ...
                                        return root.selectedCard?.model ?(
                                            <Listing
                                                root={root}
                                                modelClass={root.selectedCard?.model}
                                            />
                                        ) : (<div></div>)
                                    case 3:
                                        return (()=>{
                                            if(root.selectedCard?.view){
                                                return root.selectedCard.view(root)
                                            }
                                            return <div></div>
                                        })()
                                    default:
